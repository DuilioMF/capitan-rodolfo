...
function Resolve-UserSqlObject {
    param($Connection,[string]$Database,[string]$Name,[string]$RequestedType)
    if([string]::IsNullOrWhiteSpace($Name)){throw 'Escribí el nombre de una tabla, vista o SP.'}
    $parts=$Name.Trim().Split('.')
    if($parts.Count -eq 1){$schema='dbo';$obj=$parts[0]}
    elseif($parts.Count -eq 2){$schema=$parts[0];$obj=$parts[1]}
    else {throw 'Usá nombre o esquema.nombre; ejemplo: dbo.Tanque.'}
    if($schema -cnotmatch '^[A-Za-z_][A-Za-z0-9_]*
    param($Connection,[string]$Database)
    $Connection.ChangeDatabase($Database)
    $allowed = @(Get-SpAllowlist)
    $cmd = $Connection.CreateCommand()
...
<section class="card">
<div class="heroTop">
<div><div style="font-size:9px;letter-spacing:.22em;color:#ff9630;font-weight:900">CAPITÁN RODOLFO · NÚCLEO · DATOS</div><h1>Conexion SQL</h1><div class="muted">Servicio SQL local en segundo plano. La conexión queda guardada en esta PC.</div></div>
...
<div class="note">Si falla, el error SQL aparece arriba. Para revisar el servicio usá VER_ESTADO_SQL.bat.</div>
<button id="goMap" class="continueMap" type="button">Continuar al Mapa Vivo -></button>
<div class="note">La contraseña queda protegida por Windows. sql_profile.json se crea después de una conexión correcta y servicio_sql_estado.json muestra el estado del servicio.</div>
</section>
<section class="card">
      ...
      elseif($req.Method -eq 'POST' -and $pathOnly -eq '/api/sql/object'){
        try {
          ...
          if(-not $Sessions.ContainsKey($sid)){
            Send-Json $stream 401 @{error='Sesión vencida. Volvé a conectar en Núcleo → Datos.'}
            continue
          }
          $sess=$Sessions[$sid]
          if($sess.databases -notcontains $db){
            Send-Json $stream 403 @{error='La base de datos seleccionada no está disponible para esta sesión.'}
            continue
          }
          $action=[string]$data.action
          if($action -notin @('inspect','read','execute')){throw 'Acción no admitida.'}
          if($action -eq 'execute' -and [string]$data.confirm -cne 'EJECUTAR SP'){
            Send-Json $stream 400 @{error='Confirmación explícita requerida para ejecutar SP.'}
            continue
          }
          $res=Read-SqlObjectForUser -Connection $sess.connection -Database $db -Name ([string]$data.name) -Type ([string]$data.type) -Action $action -Parameters $data.parameters
          Send-Json $stream 200 $res
        } catch {
          Send-Json $stream 400 @{error=$_.Exception.Message}
        }
      }
...
"@
 ...
 -or $obj -cnotmatch '^[A-Za-z_][A-Za-z0-9_]*
    param($Connection,[string]$Database)
    $Connection.ChangeDatabase($Database)
    $allowed = @(Get-SpAllowlist)
    $cmd = $Connection.CreateCommand()
...
"@
  ...
  $listener.Stop()
        ...
        throw 'Ingresá un identificador simple, sin SQL libre; por ejemplo dbo.Tanque.'
    }
    $type=$RequestedType.ToLowerInvariant()
    if($type -notin @('auto','tabla','vista','sp')){throw 'Tipo no reconocido: tabla, vista o sp.'}
    $Connection.ChangeDatabase($Database)
    $cmd=$Connection.CreateCommand()
    $cmd.CommandTimeout=15
    $cmd.CommandText=@"
SELECT TOP (1) s.name AS esquema,o.name AS objeto,o.type
FROM sys.objects o INNER JOIN sys.schemas s ON s.schema_id=o.schema_id
WHERE s.name=@schema AND o.name=@obj AND o.type IN ('U','V','P')
  AND (@type='auto' OR (@type='tabla' AND o.type='U')
       OR (@type='vista' AND o.type='V') OR (@type='sp' AND o.type='P'));
"@
    $null=$cmd.Parameters.Add('@schema',[Data.SqlDbType]::NVarChar,128)
    $cmd.Parameters['@schema'].Value=$schema
    $null=$cmd.Parameters.Add('@obj',[Data.SqlDbType]::NVarChar,128)
    $cmd.Parameters['@obj'].Value=$obj
    $null=$cmd.Parameters.Add('@type',[Data.SqlDbType]::VarChar,12)
    $cmd.Parameters['@type'].Value=$type
    $reader=$cmd.ExecuteReader()
    try {
        if(-not $reader.Read()){throw "No encuentro '$Name' como $type en la base $Database. Verificá la base y el esquema."}
        $resolvedSchema=[string]$reader.GetString(0)
        $resolvedName=[string]$reader.GetString(1)
        $kind=[string]$reader.GetString(2)
    } finally {$reader.Close()}
    $full=$resolvedSchema+'.'+$resolvedName
    return @{schema=$resolvedSchema;object=$resolvedName;name=$full;kind=$kind}
}

function Read-SqlObjectForUser {
    param($Connection,[string]$Database,[string]$Name,[string]$Type,[string]$Action,$Parameters)
    $obj=Resolve-UserSqlObject -Connection $Connection -Database $Database -Name $Name -RequestedType $Type
    if($obj.kind -eq 'P'){
        $isAllowed=$false
        foreach($allowedName in @(Get-SpAllowlist)){
            if([string]::Equals($allowedName,[string]$obj.name,[StringComparison]::OrdinalIgnoreCase)){$isAllowed=$true;break}
        }
        $params=@()
        $cmd=$Connection.CreateCommand()
        $cmd.CommandText=@"
...
WHERE prm.object_id=OBJECT_ID(@name,'P')
ORDER BY prm.parameter_id;
"@
        $null=$cmd.Parameters.Add('@name',[Data.SqlDbType]::NVarChar,257)
        $cmd.Parameters['@name'].Value=$obj.name
        $reader=$cmd.ExecuteReader()
        try {
            while($reader.Read()){
                $params+=@{name=[string]$reader.GetString(0);
                    type=if($reader.IsDBNull(1)){'unknown'}else{[string]$reader.GetString(1)};
                    output=if($reader.IsDBNull(2)){$false}else{[bool]$reader.GetBoolean(2)};
                    hasDefault=if($reader.IsDBNull(3)){$false}else{[bool]$reader.GetBoolean(3)}}
            }
        } finally {$reader.Close()}
        if($Action -eq 'execute'){
            if(-not $isAllowed){
                throw 'Ese SP existe, pero no está autorizado para ejecución remota. Su ejecución podría modificar datos; agregalo conscientemente a la configuración local de SP.'
            }
            $res=Invoke-AllowedStoredProcedure -Connection $Connection -Database $Database -Procedure $obj.name -Parameters $Parameters
            return @{name=$obj.name;type='sp';database=$Database;action='execute';allowed=$true;parameters=$params;resultSets=$res.resultSets}
        }
        return @{name=$obj.name;type='sp';database=$Database;action='inspect';allowed=$isAllowed;parameters=$params;resultSets=@()}
    }
    if($Action -eq 'execute'){throw 'Las tablas y vistas solamente admiten lectura.'}
    $typeName=if($obj.kind -eq 'U'){'tabla'}else{'vista'}
    $sqlName='['+$obj.schema+'].['+$obj.object+']'
    $cmd=$Connection.CreateCommand()
    $cmd.CommandTimeout=25
    $cmd.CommandText='SELECT TOP (200) * FROM '+$sqlName+';'
    $reader=$cmd.ExecuteReader()
    $cols=@()
    $rows=New-Object System.Collections.Generic.List[object]
    try {
    ...
    return @{name=$obj.name;type=$typeName;database=$Database;action='read';
        resultSets=@(@{columns=$cols;rows=@($rows.ToArray());rowCount=$rows.Count});limit=200}
}
