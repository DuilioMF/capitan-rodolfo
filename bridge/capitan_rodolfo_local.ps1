...
<title>Capitán Rodolfo · Datos SQL</title>
<style>
:root{--bg:#090806;--panel:#12100d;--panel2:#17130f;--line:#6e583a55;--ink:#f1eadc;--muted:#9d9486;--orange:#dc8623;--gold:#cba45d;--green:#5ee6a8;--bad:#ff7a73}
*{box-sizing:border-box}html,body{margin:0;min-height:100%;background:var(--bg);color:var(--ink);font-family:Inter,Segoe UI,Arial,sans-serif}
body{background:radial-gradient(circle at 15% 0,#29180d 0,transparent 32%),radial-gradient(circle at 85% 100%,#12221d 0,transparent 30%),#090806}
.wrap{width:min(1280px,95vw);margin:18px auto 38px}.top{display:flex;justify-content:space-between;align-items:center;gap:16px;padding:10px 0 16px;border-bottom:1px solid var(--line)}
.brand b{font:600 26px Georgia,serif}.brand small{display:block;color:var(--orange);letter-spacing:.2em;font-size:9px;margin-top:3px}.ver{border:1px solid #a7683977;color:#ffb77c;padding:7px 11px;border-radius:999px;font-size:11px;font-weight:900}
.statusbar{margin:14px 0;display:grid;grid-template-columns:1fr auto;gap:12px;align-items:center;padding:12px 14px;background:#11100de8;border:1px solid var(--line);border-radius:12px}.statusbar.ok{border-color:#2d7253}.statusbar.bad{border-color:#7a3834}.statusbar strong{font-size:13px}.statusbar span{color:var(--muted);font-size:12px}
.grid{display:grid;grid-template-columns:360px 1fr;gap:16px}.card{background:#12100de8;border:1px solid var(--line);border-radius:16px;padding:18px}.card h2,.card h3{margin:0 0 10px}.eyebrow{font-size:9px;letter-spacing:.22em;color:var(--orange);font-weight:900}
label{display:block;color:var(--muted);font-size:11px;margin:11px 0 5px}input,select{width:100%;padding:11px 12px;border:1px solid #6e583a55;border-radius:9px;background:#090806;color:var(--ink);outline:none}input:focus,select:focus{border-color:#b47743}
button{border:1px solid #a7683977;border-radius:9px;padding:11px 13px;background:#2a180d;color:#ffbd84;font-weight:900;cursor:pointer}button.primary{width:100%;margin-top:13px;background:linear-gradient(135deg,#dc8623,#f3a246);color:#160d07;border:0}.secondary{background:#ffffff05;color:#ddd0bc}.row{display:flex;gap:8px;flex-wrap:wrap}.row button{flex:1}
.cred{margin-top:10px;padding:10px;border:1px dashed #6e583a55;border-radius:9px;color:var(--muted);font-size:11px;line-height:1.55}.cred.ok{color:#86e8b2;border-color:#2d7253}
.right{display:grid;grid-template-columns:1fr 1fr;gap:16px}.list{display:grid;gap:8px;max-height:260px;overflow:auto}.item,.table{padding:10px 11px;border:1px solid #4b40325e;border-radius:9px;background:#0d0c0a;color:#ddd0bc;text-align:left}.item{cursor:pointer}.item:hover,.item.active{border-color:#d9823b;background:#20140c}
.diag{margin-top:14px;padding:12px;border:1px solid #4b40325e;border-radius:10px;background:#0d0c0a;font-size:11px;line-height:1.65;color:var(--muted);white-space:pre-wrap;word-break:break-word}.diag b{color:#ffd09b}
.data{margin-top:16px}.data h3{display:flex;justify-content:space-between;gap:12px}.query{font-size:10px;color:#d69c65;font-family:Consolas,monospace}.tableWrap{overflow:auto;max-height:330px;border:1px solid #4b40325e;border-radius:10px}.dataGrid{border-collapse:collapse;width:100%;font-size:11px;min-width:680px}.dataGrid th,.dataGrid td{padding:8px 9px;border-bottom:1px solid #302920;border-right:1px solid #25211c;text-align:left;white-space:nowrap}.dataGrid th{position:sticky;top:0;background:#20140c;color:#ffb776}.dataGrid td{background:#0d0c0a}
.continueMap{display:none;width:100%;margin-top:12px}.continueMap.show{display:block}
@media(max-width:900px){.grid,.right{grid-template-columns:1fr}.wrap{width:min(96vw,760px)}}
</style>
</head>
<body>
<div class="wrap">
  <div class="top"><div class="brand"><b>Capitán Rodolfo</b><small>NÚCLEO · DATOS SQL</small></div><span class="ver">v$Version</span></div>
  <div id="serviceBar" class="statusbar"><div><strong>Servicio SQL local</strong><br><span id="serviceText">Comprobando…</span></div><div id="serviceLight">●</div></div>

  <div class="grid">
    <section class="card">
      <div class="eyebrow">CONEXIÓN</div><h2>SQL Server</h2>
      <label>Servidor / instancia</label><input id="server" value="$env:COMPUTERNAME\SQLEXPRESS" autocomplete="off">
      <label>Autenticación</label><select id="auth"><option value="sql">Usuario y contraseña SQL Server</option><option value="windows">Windows</option></select>
      <div id="sqlCreds"><label>Usuario SQL</label><input id="user" autocomplete="username"><label>Contraseña</label><input id="password" type="password" autocomplete="current-password" placeholder="Ingresala una sola vez"></div>
      <div id="credentialState" class="cred">Buscando credenciales guardadas…</div>
      <button id="connect" class="primary">Conectar y guardar</button>
      <div class="row" style="margin-top:8px"><button id="detect" class="secondary">Detectar SQL local</button><button id="refreshState" class="secondary">Ver estado</button></div>
      <div id="diag" class="diag">Estado listo para diagnóstico.</div>
      <button id="goMap" class="continueMap">Ver tanques y surtidores →</button>
    </section>

    <section>
      <div class="right">
        <div class="card"><div class="eyebrow">BASES</div><h3>Disponibles</h3><div id="dbs" class="list"><div class="item">Conectate para ver bases</div></div></div>
        <div class="card"><div class="eyebrow">TABLAS</div><h3>Base seleccionada</h3><div id="tables" class="list"><div class="table">Seleccioná una base</div></div></div>
      </div>
      <div class="card data"><h3>Tabla SURPLA <span class="query">SELECT * FROM dbo.SURPLA</span></h3><div id="surpla" class="muted">Conectate y elegí una base.</div></div>
      <div class="card data"><h3>Despachos abiertos <span class="query">estadovta = 0</span></h3><div id="dispatches" class="muted">Seleccioná una base.</div></div>
    </section>
  </div>
</div>
<script>
let sessionId=null,selectedDatabase=null;
const q=id=>document.getElementById(id);
const serverInput=q('server'),authInput=q('auth'),userInput=q('user'),passwordInput=q('password');
const dbs=q('dbs'),tables=q('tables'),surpla=q('surpla'),dispatches=q('dispatches'),goMap=q('goMap'),diag=q('diag'),cred=q('credentialState'),bar=q('serviceBar'),serviceText=q('serviceText');
function showDiag(m,ok){diag.innerHTML=(ok?'<b>OK</b> ':'<b>DIAGNÓSTICO</b> ')+String(m);diag.style.color=ok?'#86e8b2':'#d8c5b1'}
function updateAuth(){q('sqlCreds').style.display=authInput.value==='windows'?'none':'block'}
...
async function get(path){const r=await fetch(path,{cache:'no-store'});const j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j.error||('HTTP '+r.status));return j}
function showDatabases(d,autoDb){sessionId=d.sessionId;dbs.innerHTML='';(d.databases||[]).forEach(name=>{const b=document.createElement('button');b.className='item';b.textContent=name;b.onclick=()=>loadTables(name,b);dbs.appendChild(b);if(autoDb&&name===autoDb)setTimeout(()=>loadTables(name,b),0)});if(!(d.databases||[]).length)dbs.innerHTML='<div class="item">No se encontraron bases accesibles.</div>'}
async function refreshService(){
 try{const d=await get('/api/service-status');bar.className='statusbar '+(d.serviceActive?'ok':'bad');serviceText.textContent=d.serviceActive?('Activo en segundo plano · puerto '+d.port+(d.database?' · base '+d.database:'')):('Sin conexión SQL activa'+(d.lastError?' · '+d.lastError:''));showDiag('Servidor: '+(d.server||'—')+'\nUsuario: '+(d.user||'—')+'\nBase: '+(d.database||'—')+'\nContraseña protegida: '+(d.hasProtectedPassword?'sí':'no')+'\nArchivo de estado: '+d.statusFile,!!d.serviceActive);return d}catch(e){bar.className='statusbar bad';serviceText.textContent='El servicio local no responde';showDiag(e.message,false);return null}
}
async function restore(){
 try{
  const p=await get('/api/profile-status');
  if(p.saved){serverInput.value=p.server||serverInput.value;authInput.value=p.auth||'sql';userInput.value=p.user||'';updateAuth();cred.className='cred '+(p.hasPassword?'ok':'');cred.textContent=p.hasPassword?('Usuario '+(p.user||'(Windows)')+' · contraseña protegida · base '+(p.database||'sin elegir')):'Conexión guardada sin contraseña SQL.'}
  else{cred.textContent='Todavía no hay una conexión guardada. Se crea después de una conexión correcta.'}
  let state=await get('/api/state');
  if(!state.connected&&p.saved){state=await post('/api/reconnect-saved',{})}
  if(state.connected){serverInput.value=state.server||serverInput.value;authInput.value=state.auth||authInput.value;userInput.value=state.user||userInput.value;updateAuth();showDatabases(state,state.database||p.database);showDiag('SQL conectado correctamente'+(state.database?' a '+state.database:''),true)}
 }catch(e){showDiag(e.message,false)}
 await refreshService()
}
q('connect').onclick=async()=>{
 try{
  showDiag('Conectando a '+serverInput.value+'…',false);
  const payload={server:serverInput.value.trim(),auth:authInput.value,user:userInput.value.trim(),password:passwordInput.value};
  const d=await post('/api/connect',payload);passwordInput.value='';passwordInput.placeholder='Contraseña guardada y protegida por Windows';cred.className='cred ok';cred.textContent='Usuario '+(payload.user||'(Windows)')+' · contraseña guardada de forma segura';showDatabases(d,null);showDiag('Conexión SQL correcta. Elegí una base.',true);await refreshService()
 }catch(e){showDiag(e.message,false);await refreshService()}
};
q('detect').onclick=async()=>{try{const d=await get('/api/local-sql');if(d.instances&&d.instances.length){serverInput.value=d.instances[0].server;showDiag('Instancia detectada: '+d.instances.map(x=>x.server+' ['+x.status+']').join(', '),true)}else showDiag('No detecté instancias SQL Server registradas en esta PC.',false)}catch(e){showDiag(e.message,false)}};
q('refreshState').onclick=refreshService;
async function loadTables(database,el){try{selectedDatabase=database;[...document.querySelectorAll('#dbs .item')].forEach(x=>x.classList.remove('active'));el.classList.add('active');tables.innerHTML='<div class="table">Leyendo tablas…</div>';const d=await post('/api/tables',{sessionId,database});tables.innerHTML='';d.tables.forEach(t=>{const row=document.createElement('div');row.className='table';row.textContent=t.schema+'.'+t.name;tables.appendChild(row)});const result=await post('/api/surpla',{sessionId,database});renderGrid(surpla,result,'SURPLA no tiene filas.');try{const desp=await post('/api/dispatches',{sessionId,database});renderGrid(dispatches,desp,'No hay despachos abiertos.')}catch(_){dispatches.textContent='No pude leer despachos.'}goMap.classList.add('show');showDiag('Base '+database+' conectada y guardada.',true);await refreshService()}catch(e){showDiag(e.message,false)}}
function renderGrid(container,data,emptyText){container.innerHTML='';if(!data.rows||!data.rows.length){container.textContent=emptyText;return}const wrap=document.createElement('div');wrap.className='tableWrap';const table=document.createElement('table');table.className='dataGrid';const thead=document.createElement('thead'),trh=document.createElement('tr');data.columns.forEach(c=>{const th=document.createElement('th');th.textContent=c;trh.appendChild(th)});thead.appendChild(trh);table.appendChild(thead);const tbody=document.createElement('tbody');data.rows.forEach(row=>{const tr=document.createElement('tr');data.columns.forEach(c=>{const td=document.createElement('td');const v=row[c];td.textContent=(v===null||v===undefined)?'':String(v);tr.appendChild(td)});tbody.appendChild(tr)});table.appendChild(tbody);wrap.appendChild(table);container.appendChild(wrap)}
...
try {
  while($true){
    ...
    try {
      ...
      elseif($req.Method -eq 'GET' -and $pathOnly -eq '/api/service-status'){
        $st = Ensure-ActiveSession
        $p = Load-SqlProfile
        $srv = ""; $usr = ""; $db = ""; $hasProtected = $false
        if($null -ne $p){
          $srv=[string]$p.server; $usr=[string]$p.user; $db=[string]$p.database
          $hasProtected = -not [string]::IsNullOrWhiteSpace([string]$p.passwordProtected)
        }
        if($null -ne $st){
          $srv=[string]$st.server; $usr=[string]$st.user
          if(-not [string]::IsNullOrWhiteSpace([string]$st.database)){ $db=[string]$st.database }
        }
        Send-Json $stream 200 @{
          serviceActive=$true
          sqlConnected=($null -ne $st)
          version=$Version
          port=$Port
          server=$srv
          user=$usr
          database=$db
          profileExists=(Test-Path $ProfilePath)
          hasProtectedPassword=$hasProtected
          lastError=[string]$script:LastRestoreError
        ...
        foreach($regPath in @('HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server\Instance Names\SQL','HKLM:\SOFTWARE\WOW6432Node\Microsoft\Microsoft SQL Server\Instance Names\SQL')){
          try {
            ...
            foreach($prop in $props.PSObject.Properties){
              ...
              $serviceName = if($instance -eq 'MSSQLSERVER'){'MSSQLSERVER'}else{'MSSQL
        $p = Load-SqlProfile
        if($null -eq $p){
...
"@
      ...
      elseif($req.Method -eq 'POST' -and $pathOnly -eq '/api/connect'){
        try {
          ...
          $script:LastRestoreError = $_.Exception.Message
          Write-ServiceStatus -State 'running-no-sql' -Server $server -User $user -ErrorMessage $script:LastRestoreError
          Send-Json $stream 500 @{error=$_.Exception.Message}
        }
      }
...
+$instance}
              ...
              if(-not ($found | Where-Object { $_.server -eq $serverName })){
                $found.Add([pscustomobject]@{instance=$instance;server=$serverName;service=$serviceName;status=$status})
              }
            }
          } catch {}
        }
        Send-Json $stream 200 @{machine=$env:COMPUTERNAME;instances=$found}
      }
