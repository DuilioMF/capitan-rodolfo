...
echo   [2/4] Descargando conector v30...
...
echo   [4/4] Iniciando...
start "Capitan Rodolfo Local v30" /min powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%LOCALPS%"

set "OK=0"
for /L %%I in (1,1,20) do (
  powershell -NoProfile -ExecutionPolicy Bypass -Command "try { $r=Invoke-RestMethod -Uri '%HEALTH%' -TimeoutSec 1; if($r.version -eq '30'){exit 0}else{exit 1} } catch { exit 1 }"
  if not errorlevel 1 (
    set "OK=1"
    goto :ready
  )
...
echo.
echo ============================================================
echo   No se pudo iniciar el conector local v30.
echo ============================================================
echo.
